...
The `/admin/products` workspace supports product creation, editing, recent-first pagination, image/INR previews, availability, and confirmed deletion. Products persist in the existing MySQL database and can exist before being attached to an outfit. Attached products are protected from deletion.

Backend `/api/admin/products` CRUD and `/api/admin/categories` require ADMIN authorization checked in the database after JWT validation. Existing and registered users default to USER; no account is promoted automatically. User responses add a role field. Public outfit response shapes remain unchanged.

Before starting the updated backend on an existing database, apply the idempotent `backend/scripts/product-management-mysql.sql` upgrade. Follow [Product management setup](docs/product-management.md) for exact steps to promote one development account, API details, and verification. Seeding no longer modifies products belonging to existing outfits.
